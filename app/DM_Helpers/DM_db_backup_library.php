<?php
/*
     * https://github.com/marufhasan1/db_backup/blob/master/db_backup_library.php
...
namespace App\DM_Helpers;

class DM_db_backup_library{
	private $exported_database;
		
		public function connect($server,$user,$pass,$db){
			$conn=mysqli_connect($server,$user,$pass);
			$condb=mysqli_select_db($conn,$db);
			if(!$conn || !$condb){
				echo mysqli_error();
			}else{
				return $conn;
